...
public:
  ...
  GateSequencer(uint8_t p1, uint8_t p2, uint8_t p3, uint8_t p4):
    step(this), fill(this), rotation(this), mode(DISABLED), recalculate(true), lead(false),
  ...
  void update(){
    ...
    if(isTriggering())
      ...
      newmode = DISABLED;
    if(isChained())
      newmode = (GateSequencerMode)(newmode | _BV(CHAINED_BIT));
    mode = newmode;
  }
  void push(){
    if(isOn())
